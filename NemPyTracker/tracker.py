...
class Tracker ( object ):
    ...
    def __init__( self, method, src ):
        ...
        self._cap = CaptureManager( 
            cv2.VideoCapture(0),#self.captureSource), 
            self._rawWindow, 
            self.mirroredPreview, self.resolution)
 #       self._cap.setCaptureSpace(float(cv2.VideoWriter_fourcc(*'GREY')))
        self._cap.setResolution(self.resolution)
...
#                        cv2.imwrite('o-%d.jpg' % i, self.overlayImage )
                ...
                elif self.finderArgs['method'] in ['test','conf']: 
#                    self.overlayImage = copy.deepcopy(frame)
            ...
            i += 1
            self._cap.exitFrame()
            self._overlayWindow.processEvents()
